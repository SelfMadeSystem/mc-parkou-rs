        ...
        *game_mode = GameMode::Creative; // TODO: Change to adventure
        ...
        let state = GameState {
            ...
            theme: GenerationTheme::new(
                ...
                weighted_vec![(GenerationType::Single("jump_block".to_string()), 30.0),],
            ),
            score: 0,
            combo: 0,
            target_y: 0,
    ...
    for (mut client, pos, mut state, mut layer) in clients.iter_mut() {
            ...
            .position(|block| block.has_reached(*pos))
        {
            if index > 0 {
                ...
                reached_thing(&mut state, score, &mut client, pos);
            } else {
                let s = state.generations[0].has_reached_child(*pos);
                if s > 0 {
                    reached_thing(&mut state, s, &mut client, pos);
                }
            }
        }
        state.prev_player_pos = pos.0;
    }
}

fn reached_thing(
    state: &mut Mut<'_, GameState>,
    score: u32,
    client: &mut Mut<'_, Client>,
    pos: &Position,
) {
...
fn generate_next_block(state: &mut GameState, layer: &mut ChunkLayer) {
    ...
    if prev_gen.end_block.y < MIN_Y {
        state.target_y = START_POS.y;
        state.direction = JumpDirection::Up;
    } else if prev_gen.end_block.y > MAX_Y {
        state.target_y = START_POS.y;
        state.direction = JumpDirection::Down;
    } else {
        match state.direction {
            JumpDirection::Up => {
                if prev_gen.end_block.y >= state.target_y {
    ...
    let next_gen = Generator::next_in_generation(
        ...
        &state.theme,
        prev_gen,
        &state.generations.iter().collect(),
    );
